...
function keys_vote($raid)
{
    ...
    // Icon + text.
        ...
        $text_remote = EMOJI_REMOTE . getPublicTranslation('remote');
...
function checkRemote($user, $raid, $attendtime,$cbi, $plus = 0)
{
  ...
  error_log("SELECT MAX(cnt_remote) AS cnt_remote FROM (SELECT SUM(extra_mystic)+SUM(extra_valor)+SUM(extra_instinct)+SUM(remote) AS cnt_remote FROM attendance WHERE (raid_id = ".$raid." AND remote = 1 AND attend_time = '".$attendtime."') OR (raid_id = ".$raid." AND user_id = ".$user." GROUP by pokemon) AS p ")
        ...
        if ($cnt_all > 0) {
            ...
            while ($row = $rs_att->fetch_assoc()) {
				...
				if($previous_att_time == 'FIRST_RUN' && $cnt_remote > 0) {
                    $msg = raid_poll_message($msg, CR . EMOJI_REMOTE . getPublicTranslation('msg_remote_1'). EMOJI_HERE. getPublicTranslation('msg_remote_2') . CR);
                }
                ...
                if($previous_att_time != $current_att_time) {
                    ...
                    if ($cnt[$current_att_time]['count'] > 0 && $cnt[$current_att_time]['count_other_pokemon'] == 0) {
						...
						$msg = raid_poll_message($msg, (($count_remote > 0) ? EMOJI_REMOTE . $count_remote . '/ '.$config->MAX_REMOTE : ''));
                    }
                    $msg = raid_poll_message($msg, CR);
                }
